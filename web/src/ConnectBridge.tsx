...
export function ConnectBridge({ app, onRoom }: ConnectBridgeProps) {
  const [sessionId, setSessionId] = useState(() => {
    return storageGet('ac:session') || crypto.randomUUID().slice(0, 8)
  })
  const [sessionError, setSessionError] = useState('')

  const connect = () => {
    if (!SESSION_ID_RE.test(sessionId)) {
      setSessionError('Session ID must be 1-32 characters: letters, numbers, or hyphens')
      return
    }
    setSessionError('')
    storageSet('ac:session', sessionId)
